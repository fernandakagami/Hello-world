...
class Carro extends Modelo
{
    ...
    const BUSCAR_VENDIDO = 'SELECT * FROM carros WHERE vendido = true';
    const INSERIR = 'INSERT INTO carros(modelo, preco_de_compra, preco_de_venda, descricao) VALUES (?, ?, ?, ?)';
    ...
    public static function buscarVendido($id)
    {
        $comando = DW3BancoDeDados::prepare(self::BUSCAR_VENDIDO);
        $comando->bindParam(1, $id, PDO::PARAM_INT);
        $comando->execute();
        $registro = $comando->fetch();
        return new Carro(
